...
//i 为下标数，a 为首地址，d为单位空间(这里默认为整型),m1,n1为A(m1:m2,n1:n2)的索引起始值,m2,n2为A(m1:m2,n1:n2)的索引末端值
//那么此数组共有m=(m2-m1+1)行,n=(n2-n1+1)列
int calc_twoDim(int i,int j,int a,int d,int m1,int m2,int n1,int n2, bool IsRow);
//i 为下标数，a 为首地址，d为单位空间(这里默认为整型),

int main()
{
   //假设A为一个具有1000个元素的数组，每个元素为4个字节的实数，A[500]的位置为4096，请问A[1000]的地址是多少？
   calc_oneDim(1000,4096,4,500);

   //有一个PASCAL数组 A:ARRAY[6...99] of REAL(假设REAL元素存储空间的大小有4)，如果已知数组A的起始地址为500，
   //则元素A[30]的地址是多少？
   calc_oneDim(30,500,4,6);

   //现有一个二维数组A，有3x5个元素，数组的起始地址A(1,1)是100，以行为主(row-major)排列，每个元素占2个字节，
   //A(2,3)的地址是多少？
   calc_twoDim(2,3,100,2,1,3,1,5,true);

   //二维数组A[1:5,1:6],如果是以列为主(colimn-major)方式存放，则A(4,5)排在此数组的第几个位置？(a=0,d=1)
   //false时，求列数，调转i,j输入
   calc_twoDim(5,4,0,1,1,5,1,6,false);

   //数组(Array)是以PASCAL语言来声明，每个数组元素占用4个单位的内存。若起始地址是255，在下列声明中，
   //所列元素存放的位置在哪里？
   ...
   //A(-3:5,-4:2)的起始地址A(-3,-4)=1200，以行为主(row-major)排列，每个元素占1个字节，请问Loc(A(1,1))=?
   calc_twoDim(1,1,1200,1,-3,5,-4,2,true);

   //假设我们以FORTRAN语言来声明浮点数的数组A[8][10],且每个数组元素占用4个单位的内存，如果A[0][0]的起始地址是200，
   //那么元素A[5][6]的地址是多少？
   //false时，求列数，调转i,j输入
