 ...
 【双向链表】
  ...
  1.双向链表的建立与遍历
    ...
    4)建立双向链表的方法，首先就是声明每个节点有3个字段，其声明的数据结构如下所示。
     ...
     尾节点，这样的链表就称为环形双向链表。另外，为了使用方便，通常加上一个链表头指针，它的数据字段不存放任何数据，其左指针指向链表的最后一个节点，而右指针指向第一个节点(即首节点或头部节点)。建立双向链表，其实主要就是多了一个指针。
  ...
  2.双向链表中加入新节点
     ...
     2)将新节点加入双向链表的末尾:将原链表的最后一个节点的右指针指向新节点，将新节点的左指针指向原链表的最后一个节点，并将新节点的右指针指向NULL。
     C++的算法如下所示:
     ...
     3)将新节点加入到链表中的ptr节点之后:首先将ptr节点的右指针指向新节点，再将新节点的左指针指向ptr节点，接着又将
  ...
  3.双向链表节点的删除
     ...
     1)删除双向链表的第一个节点:将链表头指针head指向原链表的第二个节点，再将新链表头的左指针
     指向NULL。
     C++的算法如下:
  ...
  【链表相关应用简介】
   ...
   1)多项式表示法
      ...
      使用单向链表来表示多项式，多项式的链表表示法主要是存储非零项，且均采用COEF EXP LINK 3个字段的数据结构，其中COEF 表示非零系数，exp 表示指数的幂次(或指数)，而link 是指到下一个节点的指针。多项式的链表表示法存储了非零项。
   ...
   2）稀疏矩阵表示法
    ...
    使用链表法的最大优点就是:在变更矩阵内的数据时，不需大量移动数据。主要技巧是用节点来表示非零项，由于矩阵是二维的，因此每个节点除了必须有3个数据字段:row(行)，col(列)和value(值或数据)外，还必须有两个指针变量:right,dowm,其中right指针可用来链接同一行的节点，而down指针则用来
    链接同一列的节点。
